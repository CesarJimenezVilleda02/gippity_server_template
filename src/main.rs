...
async fn create_task(app_state: web::Data<AppState>, task: web::Json<Task>) -> impl Responder {
    ...
    db.insert(task.into_inner()); // extract task from web::Json
...
async fn update_task(app_state: web::Data<AppState>, task: web::Json<Task>) -> impl Responder {
    let mut db = app_state.db.lock().unwrap();
    db.update(task.into_inner()); // extract task from web::Json
    let _ = db.save_to_file();
    HttpResponse::Ok().finish()
}
...
async fn register(app_state: web::Data<AppState>, user: web::Json<User>) -> impl Responder {
    let mut db = app_state.db.lock().unwrap();
    db.insert_user(user.into_inner()); // extract user from web::Json
    let _ = db.save_to_file();
    HttpResponse::Ok().finish()
}
...
async fn main() -> std::io::Result<()> {
    ...
    // This is a smart pointer, when cloning we clone the reference to the data
